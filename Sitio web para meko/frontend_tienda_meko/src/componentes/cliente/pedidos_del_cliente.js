...
import Barra_lateral_como_vendedor from "../vendedores/barra_lateral_como_vendedor";
import {useEffect, useState} from "react";
function Pedidos_del_cliente (){

    const base_url = 'http://127.0.0.1:8000/api';
    const parte_de_la_url_cliente = '/cliente/';
    const parte_de_la_url_vendedor='/vendedor/';
    const parte_de_la_url_productos_del_pedido='/productos-pedidos/';
    ...
    useEffect(() => {
        fetch_data(`${base_url}${parte_de_la_url_vendedor}${vendedor_id}${parte_de_la_url_cliente}${cliente_id}${parte_de_la_url_productos_del_pedido}`);
    },[]);

    console.log("URL AKI: ",`${base_url}${parte_de_la_url_vendedor}${vendedor_id}${parte_de_la_url_cliente}${cliente_id}${parte_de_la_url_productos_del_pedido}`)
                                    ...
                                    { Productos_del_pedido.map((item,index) => {
                                        return <tr key={index}>
                                            ...
                                            <td>
                                                ...
                                                {
                                                    !item.pedidos.estado &&
                                                    <span className={"text-dark"}><i className={"fa fa-spinner spin"}></i> Pendiente</span>
...
export default Pedidos_del_cliente;
